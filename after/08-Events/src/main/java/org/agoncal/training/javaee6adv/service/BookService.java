...
public class BookService extends AbstractService<Book> implements Serializable
{
   ...
   {
      TypedQuery<String> findAllBookImagesURLQuery = getEntityManager().createQuery("SELECT DISTINCT b.imageURL FROM Book b WHERE b.imageURL != ''", String.class);
      List<String> imageURLs = findAllBookImagesURLQuery.getResultList();
      return imageURLs;
   }

   public List<Book> findByCategory(long categoryId)
   {
      TypedQuery<Book> findByCategoryQuery = getEntityManager().createQuery("SELECT DISTINCT b FROM Book b WHERE b.category.id = :categoryId", Book.class);
      findByCategoryQuery.setParameter("categoryId", categoryId);
      List<Book> booksPerCategory = findByCategoryQuery.getResultList();
