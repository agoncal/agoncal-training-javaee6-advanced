...
public class CDService extends AbstractService<CD> implements Serializable
{
   ...
   {
      TypedQuery<String> findAllBookImagesURLQuery = getEntityManager().createQuery("SELECT DISTINCT c.imageURL FROM CD c WHERE c.imageURL != ''", String.class);
   ...
   {
      TypedQuery<CD> findByGenreQuery = getEntityManager().createQuery("SELECT DISTINCT c FROM CD c WHERE c.genre.id = :genreId", CD.class);
      findByGenreQuery.setParameter("genreId", genreId);
      List<CD> booksPerCategory = findByGenreQuery.getResultList();
